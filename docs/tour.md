# Annado — Visual Tour

Every screenshot in one place, grouped by what you'd use it for. See the [README](../README.md) for the full feature documentation.

## The workflow in 90 seconds

Capture a task in an Obsidian daily note during a meeting, then edit it in Annado — side by side:

https://github.com/user-attachments/assets/343a7135-a676-4ebd-99f5-dae0626fe181


## Views

The Today view — your daily working list, grouped by project, with an Evening section and deadline flags:

![Today view](images/today.png)

The Inbox holds unscheduled captures until you process them:

![Inbox](images/inbox.png)

Anytime collects flexible tasks you can pick up whenever:

![Anytime view](images/anytime.png)

Someday is the pressure-free backlog:

![Someday view](images/someday.png)

Upcoming lays out the days ahead, with calendar events inline:

![Upcoming view](images/upcoming.png)

Everything in dark mode too:

![Today view in dark mode](images/dark-mode.png)

The Logbook keeps your completed history, grouped by day:

![Logbook](images/logbook.png)

## Plain text underneath

The same task in Annado and in the raw markdown file:

![Side-by-side with the markdown source](images/markdown-side-by-side.png)

The annotation syntax in a plain editor — task marker tags, `@when`, `@due`, Obsidian Tasks emoji dates, `!(priority)`, checklists:

![Raw markdown](images/raw-markdown.png)

An expanded task with notes and a toggleable checklist:

![Checklist](images/checklist.png)

## Capture & dates

Quick Add with scheduling, deadline, project, priority, duration, and checklist controls:

![Quick Add](images/quick-add.png)

Type a date phrase in the title and Annado offers to schedule it:

![Title hint: start date](images/date-hint-when.png)

Write "deadline tomorrow" — or "before tomorrow" — and it becomes a deadline suggestion instead:

![Title hint: deadline](images/date-hint-deadline.png)

![Title hint: deadline via "before"](images/date-hint-deadline-before.png)

Spelled-out offsets work too — "in three weeks" parses just like "in 3 weeks":

![Title hint: relative offset](images/date-hint-offset.png)

The date picker takes natural language in English and Dutch, with quick-select chips:

![Date picker](images/date-picker.png)

Quick Find searches tasks, projects, people, views, and tags as you type:

![Quick Find](images/quick-find.png)

## Organize

Color-coded projects in the sidebar:

![Projects sidebar](images/sidebar-projects.png)

A project page with its description and tasks:

![Project page](images/project-page.png)

Project metadata in action — milestones with dates, deadline, linked people:

![Project milestones](images/project-milestones.png)

Dark mode project page:

![Project page in dark mode](images/project-page-dark.png)

Smart Lists combine filters — priority, deadline window, project, person, tag, age:

![Smart List editor](images/smart-list-editor.png)

![A Smart List in the sidebar](images/smart-list.png)

Recurring templates repeat on a fixed interval or after completion:

![Recurring editor](images/recurring-editor.png)

![Recurring view](images/recurring.png)

Multi-select tasks and act on all of them at once:

![Bulk actions](images/bulk-actions.png)

People are first-class — the sidebar lists contacts with task counts:

![People sidebar](images/sidebar-people.png)

A person page shows their metadata and every linked task:

![Person page](images/person-page.png)

Tags with counts; click to filter the current view:

![Tags sidebar](images/sidebar-tags.png)

## Plan

The Agenda day view with the current-time line and your work schedule:

![Agenda day view](images/agenda-day.png)

The week view: drag tasks into slots, resize to set duration, calendar events and breaks block auto-scheduling:

![Agenda week view](images/agenda-week.png)

A second view in the side panel, with drag-and-drop between the two:

![Side panel](images/side-panel.png)

The macOS menu bar or Windows tray panel keeps today's list one click away:

![Menu bar panel](images/menu-bar.png)

## Reflect

The guided Weekly Review — process the inbox card by card with number-key actions:

![Weekly Review](images/review.png)

Wrapped sums up your week, month, or year in animated slides:

![Wrapped](images/wrapped.png)

![Wrapped year heatmap](images/wrapped-heatmap.png)

![Wrapped productivity personality](images/wrapped-personality.png)

![Wrapped outro](images/wrapped-outro.png)

## Settings

General: vault, external editor, theme, and accent color:

![Settings — General](images/settings-general.png)

Calendar: macOS calendar blocking, work schedule, and breaks that drive auto-scheduling. On Windows, Calendar integration is disabled while task scheduling remains available:

![Settings — Calendar](images/settings-calendar.png)

Shortcuts: every binding visible, customisable ones remappable:

![Settings — Shortcuts](images/settings-shortcuts.png)

Notifications: deadline reminders with per-type times, macOS menu bar or Windows tray icon, launch banner:

![Settings — Notifications](images/settings-notifications.png)
