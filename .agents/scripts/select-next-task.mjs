...
let projectSlug = readOption("--project") || "delano-multi-agent-execution";
const stream = readOption("--stream") || "default";
let tasksDir = path.join(repoRoot, ".project", "projects", projectSlug, "tasks");
if (!existsSync(tasksDir)) {
  const fallback = findFirstProjectWithTasks();
  if (fallback) {
    projectSlug = fallback.projectSlug;
    tasksDir = fallback.tasksDir;
  }
}
...
function findFirstProjectWithTasks(){
  const projectsDir = path.join(repoRoot, ".project", "projects");
  if(!existsSync(projectsDir)) return null;
  for(const projectSlug of readdirSync(projectsDir).sort()){
    const tasksDir = path.join(projectsDir, projectSlug, "tasks");
    if(existsSync(tasksDir)) return { projectSlug, tasksDir };
  }
  return null;
}
